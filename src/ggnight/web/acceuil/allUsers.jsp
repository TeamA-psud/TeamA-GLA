...
<head>
	...
	<%
		...
		ArrayList<Utilisateur> Friends = FriendFunctions.listFriend(ID_USER);
		ArrayList<Utilisateur> Attente = FriendFunctions.listAttente(ID_USER);
		System.out.println("Friends array is : "+Friends.toString());
							...
							<table id="mytable" class="table">
								...
								<tbody>
									<%for(int i=0 ; i<Friends.size() ; i++){ %>
										<tr>
											<td><%= Friends.get(i).getNom() %></td>
											<td><%= Friends.get(i).getPrenom() %></td>
											<td><%= Friends.get(i).getAge() %></td>
											<td><%= Friends.get(i).getAdresse() %></td>
										<tr>
									<%} %>
								</tbody>
							</table>
							<%} else { %>
								<h5>Aucun ami disponnible</h5>
							<%} %>
						</div>
					</div>
				</div><!-- /.panel-->
				<div class="panel panel-default">
					<div class="panel-heading">Liste des amis en attente</div>
					<div class="panel-body">
						<div class="col-md-12">
						<%if(Attente.toString() != "[]"){ %>
							<table id="mytable" class="table">
								...
								<tbody>
									<%for(int i=0 ; i<Attente.size() ; i++){ %>
										<tr>
											<td><%= Attente.get(i).getNom() %></td>
											<td><%= Attente.get(i).getPrenom() %></td>
											<td><%= Attente.get(i).getAge() %></td>
											<td><%= Attente.get(i).getAdresse() %></td>
										<tr>
									<%} %>
								</tbody>
							</table>
							<%}else{ %>
								<h5>Pas d'envoi en attente</h5>
							<%} %>
						</div>
					</div>
				</div>
