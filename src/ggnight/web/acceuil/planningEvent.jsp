...
<head>
	...
	<title>Consultation planning soirées</title>
	...
	<%
		...
		ArrayList<Utilisateur> userSendenRequest = FriendFunctions.userSendenRequest(idUser);
                ...
                ArrayList<Soiree> showSoireeConfirme = SoireeFunction.ListSoireeConfirme(idUser);
...
<body>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		...
		<div class="panel panel-container">
            ...
            <% if(allSoiree.toString() == "[]"){%>
                <h5>Il n'y'a pas de soirées organisées</h5>
            <%}else{%>
                <h5>Les soirées organisées sont :</h5>
                <table id="showUrlPlace" class="table">
                <thead>
                    <tr>
                        <th scope="col">Nom</th>
                        <th scope="col">Adresse</th>
                        <th scope="col">Date</th>
                        <th scope="col">Début Heure</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <%
                            String urlPlace = PlaceTimeFunction.getUrlPlace(allSoiree.get(i).getPlaceId());
                        %>
                        <td><input type="hidden" id="placeId" value="<%= urlPlace%>" /></td>
                    </tr>
                    <% } %>
                </tbody>
            ...
            <% if(showSoireeConfirme.toString() == "[]"){%>
                <h5>Vous n'avez pas été inviter a aucune des soirées pour le moment</h5>
            <%}else{%>
                <h5>Vous avez été inviter a les soirées suivantes</h5>
                <table id="showUrlPlace" class="table">
                <thead>
                    <tr>
                        <th scope="col">Nom</th>
                ...
                <tbody>
                    <% for(int i=0 ; i<showSoireeConfirme.size() ; i++){ %>
                    <tr>
                        <td><%= showSoireeConfirme.get(i).getNom() %></td>
                        <td><%= showSoireeConfirme.get(i).getAdresse() %></td>
                        <td><%= showSoireeConfirme.get(i).getDate() %></td>
                        <td><%= showSoireeConfirme.get(i).getdHeure() %></td>
                        <td><%= showSoireeConfirme.get(i).getfHeure() %></td>
                        <%
                            String urlPlace = PlaceTimeFunction.getUrlPlace(showSoireeConfirme.get(i).getPlaceId());
                        %>
                        <td><input type="hidden" id="placeId" value="<%= urlPlace%>" /></td>
                    </tr>
                    <% } %>
	...
	<script src="js/showUrlPlace.js"></script>		
</body>
</html>